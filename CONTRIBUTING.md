...
Pull requests targeting `main` may be opened by hand or through [`no-mistakes`](https://github.com/kunchenguid/no-mistakes).
No GitHub check requires a no-mistakes signature or attestation; `.github/workflows/ci.yml` is the only PR gate.

`no-mistakes` puts a local git proxy in front of your real remote.
Pushing through it runs an AI-driven review/test/lint pipeline in an isolated worktree and forwards the push upstream only after every check passes; opening the PR remains your call.
