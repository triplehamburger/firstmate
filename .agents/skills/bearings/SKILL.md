...
- Fill the optional `claude_usage` field from `quota-axi --provider claude --json`: the `five_hour` window is `session` and `seven_day` is `week`, each window's `percent_used` is `100 - percentRemaining`, and `session` alone also takes `resets_at` from its `resetsAt`. Pass `percent_used` as null whenever the window is missing or its `percentRemaining` is null, and omit `claude_usage` entirely when the quota read fails.
