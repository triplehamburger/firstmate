...
- Fill the optional `prs` array from the captain's open pull requests across repos with `gh search prs --author=@me --state=open --sort updated --order desc --limit 100 --json number,title,repository,url`, mapping each result's `repository.name` to `repo`, so the board's top right shows what is open right now; omit the field entirely when that query is unavailable.
- Fill the optional `claude_usage` field from `quota-axi --provider claude --json`: the `five_hour` window is `session` and `seven_day` is `week`, each window's `percent_used` is `100 - percentRemaining`, and `session` alone also takes `resets_at` from its `resetsAt`.
  Pass `percent_used` as null whenever the window is missing or its `percentRemaining` is null, and omit `claude_usage` entirely when the quota read fails.
